...
const SearchResultCard = ({ groceryStore }: Props) => {
  return (
    <Link
      to={`/detail/${groceryStore._id}`}
        ...
        <div id="card-content" className="grid md:grid-cols-2 gap-2">
          ...
          <div className="flex gap-2 flex-col">
            <div className="flex items-center gap-1 text-green-600">
              ...
              {groceryStore.estimatedDeliveryTime} mins
            </div>
            <div className="flex items-center gap-1">
              <Banknote />
              Delivery from ${(groceryStore.deliveryPrice / 100).toFixed(2)}
            </div>
          </div>
        </div>
      </div>
